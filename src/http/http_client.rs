...
///
/// The factory methods [`app`](ApiClient::app),
/// [`database`](ApiClient::database), and [`workspace`](ApiClient::workspace)
/// **consume** `self` and wrap it in an [`Arc`] stored inside the returned
/// resource handle. If you need to call account-level methods *and* create
/// resource handles from the same client, call the account-level methods
/// first:
///
/// ```no_run
...
/// let me = client.me().await?;          // borrows client
/// let app = client.app("my-app-id");    // consumes client
/// # Ok(()) }
/// ```
...
impl ApiClient {
    ...
    ///
    /// This method **consumes** `self`; see [`ApiClient`] for ownership notes.
