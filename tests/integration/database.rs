...
async fn database_metrics_returns_vec() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let result = crate::client().database(db_id).metrics().await;
...
async fn database_edit_name() {
    ...
    let result = crate::client().database(db_id).edit(None, Some(256)).await;
...
async fn database_edit_none_returns_false() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let result = crate::client().database(db_id).edit(None, None).await;
    assert_eq!(
...
async fn database_certificate_returns_string() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let result = crate::client().database(db_id).certificate().await;
...
async fn database_snapshot_lifecycle() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let db = crate::client().database(db_id);
...
async fn database_start_returns_true() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let result = crate::client().database(db_id).start().await;
...
async fn database_stop_returns_true() {
    ...
    crate::throttle().await;
    let db_id = require_db!();
    let result = crate::client().database(db_id).stop().await;
